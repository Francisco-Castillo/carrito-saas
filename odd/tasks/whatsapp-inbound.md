...
52. **`NullStockMeansInfiniteTests` es el único test que pinea la mitad del decremento para el caso nulo**, y el arreglo dependió de un accidente de SQL (`NULL - :quantity = NULL`, así que el `SET` ya era correcto y sólo la cláusula `WHERE` estaba mal). Vale tenerlo anotado porque el día que alguien cambie el `SET` —por ejemplo a `COALESCE(p.stock, x) - :quantity`— **rompe el caso infinito sin que ninguna otra cosa chille**.
