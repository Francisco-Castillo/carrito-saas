...
- [ ] **T6c — El nombre del cliente por el puerto.** `InboundMessage` gana el nombre del perfil, el traductor lo llena desde `contacts[].profile.name`, `OrderProposal` lo guarda, y el handler lo persiste. **Toca dos componentes ya verificados** (el puerto de T1 y el traductor) y el test que pinea los nombres de wire, así que la verificación de T1/T2 hay que rehacerla sobre el estado nuevo. Tests: el payload con nombre produce un `InboundMessage` con ese nombre; **un payload sin `contacts` o sin `profile` no rompe** y deja el nombre nulo (no todos los mensajes lo traen); el nombre se persiste y se relee; y el nombre sigue siendo borrador — nada lo usa como identidad del cliente.
    ...
    Tests: confirmar produce un pedido idéntico al que produciría el menú para las mismas líneas; **el doble confirmado produce UN pedido y descuenta stock UNA vez** (con prueba de sensibilidad: sin la transición condicional tiene que fallar); una línea `Suggested` no aceptada **no** llega al pedido; una `Ambiguous` tampoco; descartar no toca stock; un negocio no puede listar ni confirmar propuestas de otro; una propuesta ya decidida no se reconfirma; y una `FAILED` sin negocio no es confirmable.
